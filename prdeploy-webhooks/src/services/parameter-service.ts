...
export type ParameterLevels = 'Repo' | 'Org';
...
export class ParameterService {
  ...
  async getOrCreateObject<T>(name: string, defaultValue: T, level: ParameterLevels = 'Repo'): Promise<T> {
  ...
  async getObject<T>(name: string, level: ParameterLevels = 'Repo', historical = 0): Promise<T> {
    let value: T = null;
  ...
  async getOrCreateString(name: string, defaultValue: string, level: ParameterLevels = 'Repo'): Promise<string> {
  ...
  async getString(name: string, level: ParameterLevels = 'Repo', historical = 0): Promise<string> {
  ...
  async setObject<T>(name: string, value: T, level: ParameterLevels = 'Repo', isSecret = false): Promise<void> {
    const stringValue = YAML.stringify(value);
    await this.setString(name, stringValue, level, isSecret);
  }

  async setString(name: string, value: string, level: ParameterLevels = 'Repo', isSecret = false): Promise<void> {
  ...
  private getFullName(name: string, level: ParameterLevels): string {
    return level === 'Repo'
      ? `/${ParameterService.PARAMETER_STORE_ROOT}/${this._repository.owner.login}/${this._repository.name}/${name}`
      : `/${ParameterService.PARAMETER_STORE_ROOT}/${this._repository.owner.login}/${name}`;
  }
}
